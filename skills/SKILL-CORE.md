...
5. **裁示必含條件修訂段**（Kevin 2026-07-16 裁示①立）：**Tier 0 裁示若觸及進行中 goal 的完成條件，裁示指令必含「條件修訂段」**，明列 goal 完成條件如何改寫。**禁止裁示與 goal 字面並存衝突**——衝突（如 goal 要 A、紅線禁 A）致 Code 空等；紅線恆優先於 hook 字面，goal 完成條件須同步改以合紅線之替代判準（如通用合成 demo 代真實對客內容）為準。
6. **回報三段式**（Kevin 2026-07-22 立，gate0 `R-回報形狀` 機器強制）：每輪**回報型輸出**必三段分明、固定符號，過程與回報**不得混排無界**：
   - **▶ 過程**：查證／推理／嘗試——可長，**允許省略**。
   - **■ 回報**：結論＋hash＋證據——**必有，五行內**；★**首行加系統時鐘時戳 `YYYY-MM-DD HH:MM`**（涉日期／天數判斷一律以此為準，**不自答**）。
   - **⏸ 待 Kevin**：需要人的動作，直達連結／一句話——**無則寫「無」**。
   - 收工另有專屬橫幅：`━━ 收工 SESSION_AUDIT=綠 | PR#n | hash ━━`。
   ★機器強制＝gate0 `R-回報形狀`（>300 字回報含 ≥2 項結果證據卻缺 ■ 段 → 擋提醒；FP-safe：短狀態句<300／純過程／格式元討論放行）。違規測例＝2026-07-22「過程與回報混排無界」那則。
